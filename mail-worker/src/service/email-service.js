...
const emailService = {
	...
	async send(c, params, userId) {
		...
		let {
			...
			name, // sender name
			sendType, // send type
			emailId, // email id, present when replying
		...
		if (c.env.admin !== userRow.email) {
			...
			// Sending disabled (internal only)
		...
		// Non-admin with send count limit
		...
		if (c.env.admin !== userRow.email) {
			// User does not have permission to use this domain
		...
		// If recipients include external addresses and no sending service is configured
		if (!useCloudflareEmail && !resendToken && !allInternal) {
			throw new BizError(t('noSendProvider'));
		}

		// Auto-detect sender name if not provided
		if (!name) {
		...
		// When sending to external recipients, prefer Cloudflare Email Service, fallback to Resend
		if (!allInternal) {
		...
		// Replace cid image tags with generic URLs
		...
		// Increment user send count if role has a send limit
		...
		// If all recipients are internal, save directly to database
		if (allInternal) {
	...
	async HandleOnSiteEmail(c, receiveEmail, sendEmailData, attList) {
		...
		for (const email of receiveEmail) {
			...
			// Convert sender email to recipient view
			...
			// If recipient account exists, assign to that user
			if (accountRow) {

				// Set recipient's user info for saving
				...
				// If recipient lacks domain permission or has email blocked, mark as bounced
		...
		let message = ''
		// If any email bounced, mark sender's email as bounced
		if (bouncedEmail) {
